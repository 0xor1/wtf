...
func Everything(t *testing.T) {
	...
	tmpFirstID := id.Copy()
	defer func() {
		_, err = r.User().Primary().Exec(`DELETE FROM auths WHERE id=?`, tmpFirstID)
		PanicOn(err)
	}()
	...
	row = r.User().Primary().QueryRow(`SELECT changeEmailCode FROM auths WHERE id=?`, id)
	PanicOn(row.Scan(&code))
	...
	defer func() {
		_, err = r.User().Primary().Exec(`DELETE FROM auths WHERE id=?`, id)
		PanicOn(err)
	}()
